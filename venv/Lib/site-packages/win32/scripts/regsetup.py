...
def LocateFileName(fileNamesString, searchPaths):
    """Locate a file name, anywhere on the user path.
...
Run without options, (but possibly user paths) to repair a totally broken
...
paths are user paths that the program will use to seek out a file.
...
if __name__=='__main__':
    if len(sys.argv)>1 and sys.argv[1] in ['/?','-?','-help','-h']:
        ...
        # for developers:
        # also user somewhere\lib, ..\build, and ..\..\build
        searchPath.append("..\\..\\lib")
        searchPath.append("..\\build")
        if "64 bit" in sys.version:
